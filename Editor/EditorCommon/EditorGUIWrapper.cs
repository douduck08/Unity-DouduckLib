...
namespace DouduckLibEditor
{
    public static class EditorGUIWrapper
    {
        ...
        public static bool DrawFoldout(bool fold, string title, Action onGUI)
        {
            using (new EditorGUILayout.VerticalScope(EditorGUIStyle.Box))
            {
                fold = EditorGUILayout.Foldout(fold, title, EditorGUIStyle.Foldout);
                // GUILayout.FlexibleSpace ();
                if (fold && onGUI != null)
                    onGUI.Invoke();
                // GUILayout.FlexibleSpace ();
        ...
        public static Gradient DrawGradientField(string label, Gradient gradient, params GUILayoutOption[] options)
        {
            if (_methodInfo == null)
            {
                _methodInfo = typeof(EditorGUILayout).GetMethod("GradientField", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static, null, new[] { typeof(string), typeof(Gradient), typeof(GUILayoutOption[]) }, null);
            }
            ...
            gradient = (Gradient)_methodInfo.Invoke(null, new object[] { label, gradient, options });
            return gradient;
        }
    }
}
